...
$.fn.addtabs = function (options) {
    ...
    Addtabs.options = $.extend({
        ...
        iframeHeight: $(document).height() - 50, //固定TAB中IFRAME高度,根据需要自己修改
        method: 'init',
    ...
    $(Addtabs.options.monitor).on('click', '[data-addtab]', function () {
        Addtabs.add({
            ...
            //norefresh:$(this).attr('no-refresh')==1 ? false : true//是否允许切换TAB时进行刷新
        });
    });
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (!$("#" + id)[0]) {
            ...
            //if(opts.norefresh){
            //	$("#tab_" + id).attr("onclick","Addtabs.refresh('"+id+"','"+opts.url+"')");//添加刷新事件
            //}
        }else{
        	$("#" + id).find("iframe").attr("src",opts.url);//强制刷新
        }
    ...
    refresh: function (id,url){
    	//点击非当前激活状态的TAB，强制刷新iframe页面
    	if($("#tab_" + id).attr("class")!='active'){
